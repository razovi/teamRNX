...
class PredictiveTradeBot(AbstractTradeBot):
    ...
    def get_all_stocks_data(self):
        for stock_symbol in list(self.data_source.stocks_market_data_dict.keys()):
            self.all_stocks_symbols_data_dict[stock_symbol] = self.data_source.stocks_market_data_dict[
                stock_symbol].head()
    ...
    def performance(self):
        print("{0} final cash sum (not incl. value of remaining open orders): {1}".format(self.bot_type_name,
                                                                                          self.current_cash_sum))
